...
-   **Auto-Versioning on Deploy:** Two mechanisms: (1) For Docker deploys, use `./deploy.sh` (or `./deploy.sh docker-compose.supabase.yml`) which bumps version.json on the host before `docker compose build`. The Dockerfile sets `SKIP_VERSION_BUMP=1` to avoid double-bumping inside the container. (2) For Replit deploys, `script/build.ts` auto-bumps during `npm run build` (skipped when `SKIP_VERSION_BUMP=1`). Multiple builds on the same day append to the same changelog entry while still incrementing the patch version. Manual bumps via `scripts/bump-version.ts` (minor/major) still work.
