...
-   **Saved Predictions History:** Persistent storage for ALL AI prediction results (`saved_predictions` table) with auto-save for all prediction types (quick predictions, scenario analysis, candidate comparisons, event impacts, what-if simulations). Includes a dedicated "Histórico" tab in the predictions page with search, type filtering, expandable result previews, full detail modal, and deletion. CRUD API endpoints at `/api/saved-predictions`.
